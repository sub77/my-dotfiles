...
try:
    ...
    AVAILABLE = ['MyActivityWatch', 'MyInactivityWatch']
except (ImportError, ValueError):
    err('MyActivityWatch plugin unavailable as we cannot import Notify')

config = Config()
inactive_period = float(config.plugin_get('MyInactivityWatch', 'inactive_period',
                                        120.0))
watch_interval = int(config.plugin_get('MyInactivityWatch', 'watch_interval',
                                       5000))
hush_period = float(config.plugin_get('MyActivityWatch', 'hush_period',
                                        10.0))

class MyActivityWatch(plugin.MenuItem):
...
class MyInactivityWatch(plugin.MenuItem):
    ...
    def check_times(self, terminal):
        ...
        if time_now - self.last_activities[terminal] >= inactive_period:
            ...
            note = Notify.Notification.new(_('Terminator'), _('Silence in: %s') % 
                                         terminal.get_window_title(), 'terminator')
            note.show()
            os.system('aplay /home/sub77/.config/terminator/plugins/beep.mp3')
